...
class MountCacheService implements IEventListener {
	...
	public function registerDeletedStorage(StorageConfig $storage) {
		foreach ($this->getUsersForStorage($storage) as $user) {
			$this->userMountCache->removeMount($storage->getMountPointForUser($user));
		}
	}

	public function registerAddedStorage(StorageConfig $storage) {
		foreach ($this->getUsersForStorage($storage) as $user) {
			$this->registerForUser($user, $storage);
		}
	}

	public function registerUpdatedStorage(StorageConfig $oldStorage, StorageConfig $newStorage) {
	...
	private function registerForUser(IUser $user, StorageConfig $storage) {
